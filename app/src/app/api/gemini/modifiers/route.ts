...
사용자가 "${keyword}"를 검색할 때, 앞에 붙여서 함께 검색할 가능성이 높은 수식어(modifier)를 생성해주세요.

예시:
- "노트북" → 가성비, 추천, 순위, 가벼운, 게이밍, 사무용, 학생용, 2024, 최신
- "피자" → 맛집, 배달, 추천, 냉동, 수제, 1인, 가성비

규칙:
1. 실제 사람들이 검색할 법한 자연스러운 수식어만 생성
2. "${keyword}" 카테고리/분야에 맞는 맥락적 수식어 포함
3. 일반적인 수식어(추천, 순위, 가성비 등)도 포함
4. 20개의 수식어를 생성
